...
/*************************************************  METODOS ******************************/
void initGlobales(){
	posYDefensor = 20; /***esta pos es fija*/
	posYInvasor = 3; /***esta pos es fija*/
	MEDIO = 38;
	posXDefensor = 38;
	posXInvasor = 38;
	cantidadDiparos = 0;
	...
	comandante_defensor.x1p=3; 
	comandante_defensor.x2p=3; 
	comandante_defensor.x3p=3; 
	comandante_defensor.x4p=3; 
	comandante_defensor.x5p=3;
	comandante_defensor.x6p=3;
	comandante_defensor.x7p=3;
	comandante_defensor.y=20;
	comandante_defensor.vidas=5;
	comandante_defensor.ch1 = '<';
	comandante_defensor.ch2 = '<';
	comandante_defensor.ch3 = '=';
	comandante_defensor.ch4 = '=';
	comandante_defensor.ch5 = '=';
	comandante_defensor.ch6 = '>';
	comandante_defensor.ch7 = '>';
	comandante_invasor.x1p=3; 
	comandante_invasor.x2p=3; 
	comandante_invasor.x3p=3; 
	comandante_invasor.x4p=3; 
	comandante_invasor.x5p=3;
	comandante_invasor.x6p=3;
	comandante_invasor.x7p=3;
	comandante_invasor.y=3;
	comandante_invasor.vidas=5;
...
void jugarInvasor(WINDOW *win){
	...
	while(1) {  
	   	...
	   	++contadorTimer;
	   	key = getch();
		redibujarInvasor(win,posXInvasor,lado);
	...
	for (int i = 0; i < 10; ++i)
	{
		if(disparosInvasores[i].s == 1){
			if(disparosInvasores[i].y==19){
				if(disparosInvasores[i].x==comandante_defensor.x1p){	
				}else if (disparosInvasores[i].x==comandante_defensor.x2p){
					--comandante_defensor.vidas;
					actualizarVidas(win);
				}else if (disparosInvasores[i].x==comandante_defensor.x3p){
						--comandante_defensor.vidas;
					actualizarVidas(win);
				}else if (disparosInvasores[i].x==comandante_defensor.x4p){
						--comandante_defensor.vidas;
					actualizarVidas(win);
				}else if (disparosInvasores[i].x==comandante_defensor.x5p){
						--comandante_defensor.vidas;
					actualizarVidas(win);
				}else if (disparosInvasores[i].x==comandante_defensor.x6p){
						--comandante_defensor.vidas;
					actualizarVidas(win);
				}else if (disparosInvasores[i].x==comandante_defensor.x7p){
						--comandante_defensor.vidas;
					actualizarVidas(win);
				}
				disparosInvasores[i].s=0;
...
void redibujarDefensor(WINDOW *win , int posX, int lado){	
		...
		attron(COLOR_PAIR(3));
		/******************************************************************************************** dedede */
				move(comandante_defensor.y, comandante_defensor.x1p);
				addch(' ');
				move(comandante_defensor.y, comandante_defensor.x2p);
				addch(' ');
				move(comandante_defensor.y, comandante_defensor.x3p);
				addch(' ');
				move(comandante_defensor.y, comandante_defensor.x4p);
				addch(' ');
				move(comandante_defensor.y, comandante_defensor.x5p);
				addch(' ');
				move(comandante_defensor.y, comandante_defensor.x6p);
				addch(' ');
				move(comandante_defensor.y, comandante_defensor.x7p);
				addch(' ');
					comandante_defensor.x1 = posX;
					comandante_defensor.x2 = posX+1;
					comandante_defensor.x3 = posX+2;
					comandante_defensor.x4 = posX+3;
					comandante_defensor.x5 = posX+4;
					comandante_defensor.x6 = posX+5;
					comandante_defensor.x7 = posX+6;
				move(comandante_defensor.y, comandante_defensor.x1);
				addch(comandante_defensor.ch1);
				move(comandante_defensor.y, comandante_defensor.x2);
				addch(comandante_defensor.ch2);
				move(comandante_defensor.y, comandante_defensor.x3);
				addch(comandante_defensor.ch3);
				move(comandante_defensor.y, comandante_defensor.x4);
				addch(comandante_defensor.ch4);
				move(comandante_defensor.y, comandante_defensor.x5);
				addch(comandante_defensor.ch5);
				move(comandante_defensor.y, comandante_defensor.x6);
				addch(comandante_defensor.ch6);
				move(comandante_defensor.y, comandante_defensor.x7);
				addch(comandante_defensor.ch7);
					comandante_defensor.x1p = comandante_defensor.x1;
					comandante_defensor.x2p = comandante_defensor.x2;
					comandante_defensor.x3p = comandante_defensor.x3;
					comandante_defensor.x4p = comandante_defensor.x4;
					comandante_defensor.x5p = comandante_defensor.x5;
					comandante_defensor.x6p = comandante_defensor.x6;
					comandante_defensor.x7p = comandante_defensor.x7;
...
void generarDisparo(WINDOW *win){
	...
	if(hayEspacio==1){
		disparos[i].x = posXDefensor + 3;
		disparos[i].y = posYDefensor-1;
		disparos[i].s = 1;
		disparos[i].ch = '*';
		//actualizarDisparos (win);
        attron(COLOR_PAIR(4));
		move(disparos[i].y,disparos[i].x);
        addch(disparos[i].ch);
...
void actualizarDisparos(WINDOW *win){
	for (int i = 0; i < 10; ++i)
	{
		if(disparos[i].s == 1){
			if(disparos[i].y==4){
				if(disparos[i].x==comandante_invasor.x1p){	
				}else if (disparos[i].x==comandante_invasor.x2p){
					--comandante_invasor.vidas;
					actualizarVidas(win);
				}else if (disparos[i].x==comandante_invasor.x3p){
						--comandante_invasor.vidas;
					actualizarVidas(win);
				}else if (disparos[i].x==comandante_invasor.x4p){
						--comandante_invasor.vidas;
					actualizarVidas(win);
				}else if (disparos[i].x==comandante_invasor.x5p){
						--comandante_invasor.vidas;
					actualizarVidas(win);
				}else if (disparos[i].x==comandante_invasor.x6p){
						--comandante_invasor.vidas;
					actualizarVidas(win);
				}else if (disparos[i].x==comandante_invasor.x7p){
						--comandante_invasor.vidas;
					actualizarVidas(win);
				}
				disparos[i].s=0;
				...
				if(disparos[i].y>4){
					...
					move(disparos[i].y,disparos[i].x);
			        addch(' ');
					disparos[i].y = disparos[i].y-1;
					if(disparos[i].y==5){
						if(MJ[0][disparos[i].x-regresarParaMJ]==0){
							move(disparos[i].y,disparos[i].x);
			        		addch(disparos[i].ch);
						}else{
							//printf("%i\n",MJ[0][disparos[i].x-regresarParaMJ]);
							disparos[i].s=0;
							DestruirNave(win,MJ[0][disparos[i].x-regresarParaMJ]);
						}
					}else if(disparos[i].y==7){
						if(MJ[1][disparos[i].x-regresarParaMJ]==0){
							move(disparos[i].y,disparos[i].x);
			        		addch(disparos[i].ch);
						}else{
							//printf("%i\n",MJ[1][disparos[i].x-regresarParaMJ]);
							disparos[i].s=0;
							DestruirNave(win,MJ[1][disparos[i].x-regresarParaMJ]);
						}
					}else if(disparos[i].y==9){
						if(MJ[2][disparos[i].x-regresarParaMJ]==0){
							move(disparos[i].y,disparos[i].x);
			        		addch(disparos[i].ch);
						}else{
							//printf("%i\n",MJ[2][disparos[i].x-regresarParaMJ]);
							disparos[i].s=0;
							DestruirNave(win,MJ[2][disparos[i].x-regresarParaMJ]);
						}
					}else if(disparos[i].y==11){
						if(MJ[3][disparos[i].x-regresarParaMJ]==0){
							move(disparos[i].y,disparos[i].x);
			        		addch(disparos[i].ch);
						}else{
							//printf("%i\n",MJ[3][disparos[i].x-regresarParaMJ]);
							disparos[i].s=0;
							DestruirNave(win,MJ[3][disparos[i].x-regresarParaMJ]);
						}
					}else{
						move(disparos[i].y,disparos[i].x);
			        	addch(disparos[i].ch);
...
void pintarCuadroJuego(WINDOW *win){
	...
	attroff(COLOR_PAIR(2));
	attron(COLOR_PAIR(4));
	mvwprintw(win, 24-2, 8, "%d", puntajeDefensor);
	attroff(COLOR_PAIR(4));
...
/************************** compartidos ******/
	...
	void actualizarTimer(WINDOW *win){
	        ...
	        mvwprintw(win, 1, 9, "%d", minuts);
	        mvwprintw(win, 1, 10, "%s", ":");
	        mvwprintw(win, 1, 11, "%d", seconds);
			attroff(COLOR_PAIR(4));
			refresh();
	}
	...
	void actualizarPuntajeDefensor(WINDOW *win, int tipo){
		if(tipo==0){
			puntajeDefensor = puntajeDefensor + 10;
			attron(COLOR_PAIR(4));
				mvwprintw(win, 24-2, 8, "%d", puntajeDefensor);
		   	attroff(COLOR_PAIR(4));
		}else{
			puntajeDefensor = puntajeDefensor + 15;
			attron(COLOR_PAIR(4));
			mvwprintw(win, 24-2, 8, "%d", puntajeDefensor);
		   	attroff(COLOR_PAIR(4));
		}
	...
	void actualizarVidas(WINDOW *win){
			attron(COLOR_PAIR(4));
			mvwprintw(win, 1, 75, "%i", comandante_invasor.vidas);
			mvwprintw(win, 24-2, 76, "%i" ,comandante_defensor.vidas);
		   	attroff(COLOR_PAIR(4));
